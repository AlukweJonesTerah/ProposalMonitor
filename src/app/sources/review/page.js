...
export default function SourceReview() {
  ...
  const load = () => fetch('/api/sources', { cache: 'no-store' }).then((response) => response.json()).then(setData);
  useEffect(() => { load().catch(() => setMessage('Could not load submitted sources.')); }, []);
  const request = async (payload) => {
    setMessage(''); const response = await fetch('/api/sources', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); const result = await response.json();
    setMessage(response.ok ? (result.message || 'Source updated.') : (result.error || 'Could not update this source.'));
    if (response.ok) { setNewUrl(''); load(); }
  };
