...
function fallbackAnalysis(url, html, content) {
  ...
  const category = /digital health|healthcare|telemedicine|climate technology|smart cities/.test(text) ? 'Digital Health & Climate Tech' : /youth employment|youth digital skills|women in technology|women in stem|gender inclusion/.test(text) ? 'Youth, Women & Inclusion' : /call for papers|paper proposal/.test(text) ? 'Paper proposal' : /certif/.test(text) ? 'Certification' : /grant|funding/.test(text) ? 'Grant' : /data science|data scientist|machine learning|artificial intelligence/.test(text) ? 'Data science' : /analysis|analytics|analytical/.test(text) ? 'Analytics' : /training/.test(text) ? 'Training' : 'Other';
...
// Each tenant records its one-off analyses to its own file (shown in its own
// Source Review "One-off links analysed" section), but the fetch/prompt/
// fallback logic is identical, so it lives here once per prefix.
export function createAnalyseHandler(prefix = '') {
  const { analysis: analysisFile } = tenantPaths(prefix);
